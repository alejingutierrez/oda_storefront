...
Remediación masiva balanceada: `POST /api/admin/catalog-refresh/remediate` (request `{ dryRun, strategy, limit, minNoProgressMinutes, pauseOverCapTarget }`) ejecuta reconcile previo + resume por lotes + pausa selectiva sobre cap. El cron `/api/admin/catalog-refresh/cron` ahora incluye bloque `stuckRemediation` y `GET /api/admin/catalog-refresh/state` agrega `summary.processingNoProgressCount`, `summary.processingNoProgressTop`, `summary.processingNoProgressOverflow` y `diagnostics.processingNoProgress`.
...
node scripts/backfill-variant-images-to-blob.mjs  # migra `variants.images[]` externos a Blob por lotes
	...
	node scripts/rebaseline-catalog-refresh-metadata.mjs --apply  # aplica el rebaseline en DB (usar después de validar dry-run)
		...
		node scripts/monitor-catalog-refresh-ops.mjs --base-url=https://oda-moda.vercel.app --interval-minutes=10 --iterations=24 --run-cron --remediate-cooldown-minutes=30 --reconcile-job-scan-limit=2500 --reconcile-reenqueue-limit=800 --reconcile-include-active-analysis=true --state-file=../../reports/catalog_refresh_diagnostics/monitor-state.json  # monitoreo continuo + auto-reconcile/remediate con snapshots (persistiendo cooldown entre ejecuciones)
