...
- OAuth callback hardening: si Descope vuelve a `/sign-in` con `?code=` o `?err=`, la UI espera bootstrap de sesión y hace recuperación silenciosa por token/sesión (sin `sdk.oauth.exchange(code)` manual). `E061301` se trata como carrera recuperable (sin botón `Reintentar` y sin error visible), redirigiendo a `/auth/callback` cuando aparece sesión o limpiando la URL de forma automática.
