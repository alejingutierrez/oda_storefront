...
Semántica KPI (admin refresh): `freshBrands`/`staleBrands` ahora representan cobertura operativa automática (último intento finalizado en ventana semanal para marcas auto‑elegibles). Además, `GET /api/admin/catalog-refresh/state` expone `operationalFreshBrands`, `qualityFreshBrands`, `operationalStaleBrands`, `qualityStaleBrands`, `autoEligibleBrands`, `staleBreakdown`, `operationalMissingBrands[]`, `oldestOperationalRefresh`, y `diagnostics` con `queueDrift`, `heartbeat` y `progress` efectivo reciente. Por marca agrega `schedulerDue`, `operationalOverdue`, `lastOperationalAt`, `lastForceAttemptAt`, `lastForceResult`, `runStatus` y `runUpdatedAt`; también devuelve `criticalOperationalAlerts` (sin truncar) y mantiene `alerts` (compat, truncado). Cada marca faltante incluye `runProgress` (runId/status/completed/failed/pending/progressPct). Las alertas priorizan `processing` con progreso real reciente vs `processing` sin progreso y drift.
...
- Panel `/admin/catalog-refresh`:
  ...
  - Columna de estado distingue `runStatus/refreshStatus` para evitar ambigüedad cuando calidad gate marca la corrida en `failed`.
