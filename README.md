...
Remediación masiva (agresiva/balanceada): `POST /api/admin/catalog-refresh/remediate` soporta `strategy=balanced|aggressive_tail_close`. Request base: `{ dryRun, strategy, limit, minNoProgressMinutes, pauseOverCapTarget }`. En modo agresivo acepta además `{ tailProgressPct, tailMaxRemaining, tailStaleMinutes, forceTerminalizeRemaining, runId }` y responde contadores de cierre (`forcedClosedRuns`, `forcedFailedItems`, `tailCandidates`, `tailProcessedRuns`). El cron adaptable (`/api/admin/catalog-refresh/cron?mode=light|heavy`) incluye bloque `stuckRemediation` y `GET /api/admin/catalog-refresh/state` agrega telemetría de stuck alto progreso (`summary.highProgressNoProgressCount/highProgressNoProgressTop`) y capacidad real de scheduling (`processingRunCount`, `pausedRunCount`, `blockedRunCount`, `schedulingCapacityRemaining`).
...
- Health: `GET /api/admin/queue-health` (jobCounts BullMQ + workerAlive + `flags.heartbeatMissing/activeHung/queueDriftDetected/aggressiveRecoveryRequired` + `drift` catálogo + breakdown de zombies activos por estado DB).
