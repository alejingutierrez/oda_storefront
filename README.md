...
node scripts/backfill-variant-images-to-blob.mjs  # migra `variants.images[]` externos a Blob por lotes
	...
	node scripts/rebaseline-catalog-refresh-metadata.mjs --apply  # aplica el rebaseline en DB (usar después de validar dry-run)
		...
		node scripts/monitor-catalog-refresh-ops.mjs --base-url=https://oda-moda.vercel.app --interval-minutes=5 --iterations=0 --run-cron --cron-mode=light --intervention-mode=minimal --near-close-progress-pct=90 --near-close-max-pending=40 --near-close-run-limit=8 --near-close-min-consecutive-cycles=2 --remediate-cooldown-minutes=30 --remediate-limit=60 --remediate-min-no-progress-minutes=20 --remediate-pause-over-cap-target=48 --remediate-tail-progress-pct=99 --remediate-tail-max-remaining=20 --remediate-tail-stale-minutes=20 --remediate-force-terminalize-remaining=true --guardrail-consecutive-cycles=2 --heartbeat-guardrail-consecutive-cycles=2 --guardrail-drain-limit=12 --guardrail-drain-max-ms=15000 --reconcile-job-scan-limit=2500 --reconcile-reenqueue-limit=800 --reconcile-include-active-analysis=true --state-file=../../reports/catalog_refresh_diagnostics/monitor-state.json --lock-file=../../reports/catalog_refresh_diagnostics/monitor.lock  # monitoreo continuo 24/7 con baja intervención y diagnóstico persistente
