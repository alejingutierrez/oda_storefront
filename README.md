...
- Redis: `REDIS_URL`.
- Storage: `VERCEL_BLOB_READ_WRITE_TOKEN`, `BLOB_READ_WRITE_TOKEN`, `BLOB_UPLOAD_TIMEOUT_MS` (default `45000`), `ALLOW_EXTERNAL_MEDIA_WRITE` (default `false`), `IMAGE_PROXY_LIST_FALLBACK` (default `false`), `BLOB_ENABLE_LIST_FALLBACK` (default `false`).
...
- Catalog refresh semanal: `CATALOG_REFRESH_INTERVAL_DAYS`, `CATALOG_REFRESH_JITTER_HOURS`, `CATALOG_REFRESH_MAX_BRANDS`, `CATALOG_REFRESH_MAX_ACTIVE_RUNS`, `CATALOG_REFRESH_BRAND_CONCURRENCY`, `CATALOG_REFRESH_MAX_RUNTIME_MS`, `CATALOG_REFRESH_MIN_GAP_HOURS`, `CATALOG_REFRESH_MAX_FAILED_ITEMS` (default `30`), `CATALOG_REFRESH_MAX_FAILED_RATE` (default `0.10`), `CATALOG_REFRESH_FAILED_BACKOFF_BASE_HOURS` (default `6`), `CATALOG_REFRESH_FAILED_BACKOFF_MAX_HOURS` (default `72`), `CATALOG_REFRESH_DISCOVERY_LIMIT` (0 = sin límite), `CATALOG_REFRESH_FORCE_DISCOVERY_LIMIT` (default `400`), `CATALOG_REFRESH_FORCE_DISCOVERY_BUDGET_MS` (default `10000`), `CATALOG_WOO_DISCOVERY_MAX_PAGES` (default `20`), `CATALOG_REFRESH_COVERAGE_ENABLED`, `CATALOG_REFRESH_AUTO_RECOVER`, `CATALOG_REFRESH_RECOVER_MAX_RUNS`, `CATALOG_REFRESH_RECOVER_STUCK_MINUTES`, `CATALOG_REFRESH_ENRICH_RECOVER_STUCK_MINUTES`, `CATALOG_REFRESH_FAILED_LOOKBACK_DAYS`, `CATALOG_REFRESH_FAILED_URL_LIMIT`, `CATALOG_REFRESH_ENRICH_LOOKBACK_DAYS`, `CATALOG_REFRESH_ENRICH_MAX_PRODUCTS`, `CATALOG_REFRESH_ENRICH_AUTO_START`, `CATALOG_REFRESH_AUTO_RECONCILE`, `CATALOG_REFRESH_AUTO_RECONCILE_JOB_SCAN_LIMIT`, `CATALOG_REFRESH_AUTO_RECONCILE_REENQUEUE_LIMIT`, `CATALOG_REFRESH_DRAIN_ON_RUN`, `CATALOG_REFRESH_REQUIRE_BLOB_IMAGES` (default `true`), `CATALOG_REFRESH_AUTO_REMEDIATE_MAX_BRANDS` (default `4`), `CATALOG_REFRESH_AUTO_REMEDIATE_COOLDOWN_HOURS` (default `12`), `CATALOG_REFRESH_AUTO_FINALIZE_LIGHT_LIMIT` (default `5`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_ENABLED` (default `true`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_MIN_COMPLETED_RUNS` (default `2`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_WINDOW_DAYS` (default `21`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_RELAXED_ENABLED` (default `true`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_RELAXED_MIN_COMPLETED_RUNS` (default `1`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_RELAXED_WINDOW_DAYS` (default `45`), `CATALOG_MANUAL_REVIEW_AUTOCLEAR_RELAXED_MAX_FAILED_RATE` (default `0.10`), `CATALOG_ALERT_STUCK_MINUTES`, `CATALOG_REFRESH_DRIFT_SAMPLE_LIMIT`, `CATALOG_PROGRESS_WINDOW_MINUTES`, `PRODUCT_ENRICHMENT_ALERT_STUCK_MINUTES`, `CATALOG_REFRESH_ARCHIVE_AUTOMATION_ENABLED`, `CATALOG_REFRESH_ARCHIVE_AUTOMATION_DRY_RUN`, `CATALOG_REFRESH_ARCHIVE_AUTOMATION_LIMIT`, `CATALOG_ARCHIVE_POLICY_VERSION`, `CATALOG_ARCHIVE_404_MIN_ITEMS`, `CATALOG_ARCHIVE_404_MIN_RATIO`, `CATALOG_ARCHIVE_404_RECHECK_SAMPLE_LIMIT`, `CATALOG_ARCHIVE_404_RECHECK_MIN_RATIO`, `CATALOG_ARCHIVE_NO_PRODUCTS_MIN_VALIDATIONS`, `CATALOG_ARCHIVE_NO_PRODUCTS_MIN_VALIDATION_HOURS`, `CATALOG_ARCHIVE_NO_PRODUCTS_DISCOVERY_LIMIT`, `CATALOG_ARCHIVE_HTTP_TIMEOUT_MS`.
- Catalog refresh (remediación continua): `CATALOG_ALERT_TOP_BRANDS` (default `25`), `CATALOG_ALERT_HIGH_PROGRESS_THRESHOLD` (default `95`), `CATALOG_QUEUE_ACTIVE_COMPLETED_GRACE_SECONDS` (default `180`), `CATALOG_REFRESH_STUCK_REMEDIATE_ENABLED` (default `true`), `CATALOG_REFRESH_STUCK_REMEDIATE_STRATEGY` (`balanced|aggressive_tail_close|safe_fast_high_progress`, default `aggressive_tail_close`), `CATALOG_REFRESH_STUCK_REMEDIATE_LIMIT` (default `30`), `CATALOG_REFRESH_STUCK_REMEDIATE_WINDOW_MINUTES` (default `30`), `CATALOG_REFRESH_STUCK_REMEDIATE_THRESHOLD` (default `30`), `CATALOG_REFRESH_STUCK_PAUSE_OVER_CAP_ENABLED` (default `true`), `CATALOG_REFRESH_STUCK_PAUSE_OVER_CAP_TARGET` (default `48`), `CATALOG_REFRESH_STUCK_TAIL_PROGRESS_PCT` (default `95`), `CATALOG_REFRESH_STUCK_TAIL_MAX_REMAINING` (default `0`, desactivado), `CATALOG_REFRESH_STUCK_TAIL_STALE_MINUTES` (default `60`), `CATALOG_REFRESH_STUCK_TAIL_FORCE_TERMINALIZE_REMAINING` (default `true`), `CATALOG_REFRESH_HIGH_PROGRESS_MIN_PCT` (default `95`), `CATALOG_REFRESH_HIGH_PROGRESS_MAX_PENDING` (default `10`), `CATALOG_REFRESH_HIGH_PROGRESS_NO_PROGRESS_MINUTES` (default `5`), `CATALOG_REFRESH_HIGH_PROGRESS_CLOSE_LIMIT` (default `20`), `CATALOG_REFRESH_HIGH_PROGRESS_FORCE_TERMINALIZE_REMAINING` (default `true`), `CATALOG_REFRESH_CONTINUOUS_RESEED_ENABLED` (default `true`), `CATALOG_REFRESH_CONTINUOUS_TRIGGER_COVERAGE_PCT` (default `100`), `CATALOG_REFRESH_CONTINUOUS_MAX_WAITING` (default `500`), `CATALOG_REFRESH_CONTINUOUS_REQUIRE_HEALTHY_QUEUE` (default `true`), `CATALOG_REFRESH_CONTINUOUS_MIN_GAP_HOURS` (default `12`), `CATALOG_REFRESH_SPEED_SLA_ITEMS_5M` (default `120`), `CATALOG_REFRESH_SPEED_WARN_CYCLES` (default `2`), `CATALOG_REFRESH_SPEED_CRITICAL_CYCLES` (default `3`), `CATALOG_REFRESH_SPEED_CRITICAL_ITEMS_5M` (default `60`), `CATALOG_REFRESH_MONITOR_INTERVENTION_MODE` (`observe|minimal|active`, default `minimal`; recomendado `active` en producción), `CATALOG_REFRESH_MONITOR_NEAR_CLOSE_PROGRESS_PCT` (default `95`), `CATALOG_REFRESH_MONITOR_NEAR_CLOSE_MAX_PENDING` (default `40`), `CATALOG_REFRESH_MONITOR_NEAR_CLOSE_RUN_LIMIT` (default `8`), `CATALOG_REFRESH_MONITOR_NEAR_CLOSE_MIN_CONSECUTIVE_CYCLES` (default `2`), `CATALOG_REFRESH_MONITOR_HIGH_PROGRESS_CLOSE_LIMIT_PER_CYCLE` (default `12`), `CATALOG_REFRESH_MONITOR_HIGH_PROGRESS_CLOSE_COOLDOWN_MINUTES` (default `5`), `CATALOG_REFRESH_MONITOR_REMEDIATE_LIMIT` (default `60`), `CATALOG_REFRESH_MONITOR_REMEDIATE_MIN_NO_PROGRESS_MINUTES` (default `20`), `CATALOG_REFRESH_MONITOR_REMEDIATE_PAUSE_OVER_CAP_TARGET` (default `64`), `CATALOG_REFRESH_MONITOR_GUARDRAIL_CONSECUTIVE_CYCLES` (default `2`), `CATALOG_REFRESH_MONITOR_HEARTBEAT_GUARDRAIL_CONSECUTIVE_CYCLES` (default `2`), `CATALOG_REFRESH_MONITOR_GUARDRAIL_DRAIN_LIMIT` (default `12`), `CATALOG_REFRESH_MONITOR_GUARDRAIL_DRAIN_MAX_MS` (default `15000`), `CATALOG_REFRESH_MONITOR_STATE_FILE`, `CATALOG_REFRESH_MONITOR_LOCK_FILE`, `CATALOG_REFRESH_MONITOR_CRON_MODE` (default `light`), `CATALOG_REFRESH_STATE_DEEP_CACHE_TTL_SECONDS`, `ADMIN_QUEUE_HEALTH_CACHE_TTL_SECONDS`.
...
Tip operativo: `GET /api/admin/catalog-refresh/cron` acepta `mode=light|heavy` (default `light`) y overrides opcionales `maxBrands`, `brandConcurrency` y `maxRuntimeMs` para pruebas controladas de throughput (por ejemplo: `/api/admin/catalog-refresh/cron?mode=light&force=true&maxBrands=12&brandConcurrency=4&maxRuntimeMs=90000`). Si no envías esos query params, el endpoint usa los valores de entorno (`CATALOG_REFRESH_*`) sin aplicar overrides.
Semántica KPI (admin refresh): `freshBrands`/`staleBrands` representan cobertura operativa automática (último intento finalizado en ventana semanal para marcas auto‑elegibles). `GET /api/admin/catalog-refresh/state` ahora agrega estado canónico por marca (`catalogStatus`) y diagnóstico (`statusDiagnostics.runStatus/refreshStatus/source`), además de `summary.operationalCoverageExact`, `summary.operationalHealthOk`, `summary.operational100Real`, `summary.operationalCoveragePctRaw`, `summary.operationalCoveragePctDisplay`, `summary.realGapReasons`, `summary.statusMismatchCount` y `summary.statusMismatchSample`. Se mantienen `runStatus` y `refresh.lastStatus` para compatibilidad. También conserva `criticalOperationalAlerts` (sin truncar), `alerts` (compat/truncado), `operationalMissingBrands[]` con `runProgress`, y `diagnostics` de drift/heartbeat/progreso efectivo.
Semántica stale (admin refresh): `summary.staleBreakdown` separa `processing`, `failed`, `completed_stale`, `unknown`, `manual_review`. `no_status` se mantiene como alias de compatibilidad (`completed_stale + unknown`).
Control de presión recomendado en prod: comenzar con `CATALOG_REFRESH_MAX_ACTIVE_RUNS=24` y `CATALOG_REFRESH_MAX_BRANDS=12`; si en 6-12h persiste backlog alto, subir primero `CATALOG_WORKER_CONCURRENCY` antes de aumentar `maxBrands`.
Remediación masiva y cierre high-progress: `POST /api/admin/catalog-refresh/remediate` soporta `strategy=balanced|aggressive_tail_close|safe_fast_high_progress`. Request base: `{ dryRun, strategy, limit, minNoProgressMinutes, pauseOverCapTarget }`. Estrategia `aggressive_tail_close`: `{ tailProgressPct, tailMaxRemaining, tailStaleMinutes, forceTerminalizeRemaining, runId }`. Estrategia `safe_fast_high_progress`: `{ highProgressMinPct, highProgressMaxPending, highProgressNoProgressMinutes, forceTerminalizeRemaining, runId }`. Respuesta expandida con contadores: `forcedClosedRuns`, `forcedFailedItems`, `tailCandidates`, `tailProcessedRuns`, `highProgressCandidates`, `highProgressProcessedRuns`, `highProgressClosedRuns`, `highProgressForcedFailedItems`, `highProgressSampleRuns`. `GET /api/admin/catalog-refresh/state` expone `summary.throughput` (`itemsCompleted5m`, `itemsCompletedPerMinute5m`, `runsClosed5m`, `slaTargetItems5m`, `slaMet`) y `summary.highProgressClosableTop`; `GET /api/admin/queue-health` expone `throughput.itemsCompleted5m/itemsPerMinute5m/queueBurnEstimateMinutes/catalogNoProgressMinutes`.
Force individual asíncrono (sin 504): `POST /api/admin/catalog-refresh/force` con `{ "brandId": "<uuid>", "force": true }` responde `202` con `{ accepted, mode, runId, pollUrl }`.
Guardrail de timestamps refresh/enrichment: antes de normalizar a `timestamptz`, correr `node apps/web/scripts/precheck-refresh-timestamp-offset.mjs --json`; si `consistent=false`, abortar normalización de datos y aplicar mitigación de lectura hasta estabilizar offset.
...
- `services/worker` ahora valida heartbeat al boot (fail-fast si no puede escribir TTL en Redis), usa timeout HTTP finito por default (`WORKER_FETCH_TIMEOUT_MS=120000`), y reporta señales de `activeHung`/`heartbeatMissing` cuando aplica.
...
- Timeout HTTP del worker hacia `process-item`: `WORKER_FETCH_TIMEOUT_MS` (default `120000`) y overrides por cola (`CATALOG_WORKER_FETCH_TIMEOUT_MS`, `PRODUCT_ENRICHMENT_WORKER_FETCH_TIMEOUT_MS`, `PLP_SEO_WORKER_FETCH_TIMEOUT_MS`). Si alguno queda en `0`, el worker mantiene watchdog activo (`WORKER_ACTIVE_WATCHDOG_MS`, default `WORKER_ACTIVE_HUNG_MINUTES * 60s`) para evitar jobs `active` indefinidos.
- Override manual: `POST /api/admin/catalog-extractor/drain?force=true` o `POST /api/admin/product-enrichment/drain?force=true`.
- Health: `GET /api/admin/queue-health` (jobCounts BullMQ + workerAlive + `flags.heartbeatMissing/activeHung/queueDriftDetected/aggressiveRecoveryRequired` + `drift` catálogo + breakdown de zombies activos por estado DB + bloque `throughput` con SLA y estimación de vaciado de cola).
- Ejemplos de `systemd`: worker `services/worker/systemd/oda-worker.service.example` + `services/worker/systemd/worker.env.example`, y monitor 24/7 `services/worker/systemd/oda-catalog-refresh-monitor.service.example` + `services/worker/systemd/catalog-refresh-monitor.env.example`.
...
node scripts/backfill-variant-images-to-blob.mjs  # migra `variants.images[]` externos a Blob por lotes
	...
	node scripts/rebaseline-catalog-refresh-metadata.mjs --apply  # aplica el rebaseline en DB (usar después de validar dry-run)
		node scripts/reconcile-catalog-refresh-status.mjs --dry-run --days 45  # diagnostica y reconcilia lastStatus/lastFinishedAt metadata vs último run (sin escribir)
		node scripts/reconcile-catalog-refresh-status.mjs --apply --days 45  # aplica reconciliación histórica de metadata catalog_refresh
		node scripts/monitor-catalog-refresh-ops.mjs --base-url=https://oda-moda.vercel.app --interval-minutes=5 --iterations=0 --run-cron --cron-mode=light --intervention-mode=active --near-close-progress-pct=95 --near-close-max-pending=40 --near-close-run-limit=8 --near-close-min-consecutive-cycles=2 --high-progress-min-pct=95 --high-progress-max-pending=10 --high-progress-no-progress-minutes=5 --high-progress-close-limit-per-cycle=12 --high-progress-close-cooldown-minutes=5 --speed-sla-items-5m=120 --speed-warn-cycles=2 --speed-critical-cycles=3 --speed-critical-items-5m=60 --remediate-cooldown-minutes=30 --remediate-limit=60 --remediate-min-no-progress-minutes=20 --remediate-pause-over-cap-target=48 --remediate-tail-progress-pct=95 --remediate-tail-max-remaining=0 --remediate-tail-stale-minutes=60 --remediate-force-terminalize-remaining=true --guardrail-consecutive-cycles=2 --heartbeat-guardrail-consecutive-cycles=2 --guardrail-drain-limit=12 --guardrail-drain-max-ms=15000 --reconcile-job-scan-limit=2500 --reconcile-reenqueue-limit=800 --reconcile-include-active-analysis=true --state-file=../../reports/catalog_refresh_diagnostics/monitor-state.json --lock-file=../../reports/catalog_refresh_diagnostics/monitor.lock  # monitoreo continuo 24/7 con cierre autónomo high-progress y SLA de velocidad
		node scripts/monitor-catalog-refresh-ops.mjs --base-url=https://oda-moda.vercel.app --interval-minutes=5 --iterations=0 --intervention-mode=observe --state-file=../../reports/catalog_refresh_diagnostics/monitor-state.json --lock-file=../../reports/catalog_refresh_diagnostics/monitor.lock  # modo observación (sin acciones), útil para aislar causa raíz
		node scripts/auto-clear-manual-review.mjs --dry-run  # evalúa auto-conversión manual->automático por evidencia operativa
	node scripts/auto-clear-manual-review.mjs --apply  # aplica auto-clear de manualReview (si cumple 2 runs exitosos / 21 días)
	node scripts/recover-bullmq-queues.mjs --dry-run  # obliterate BullMQ + reset DB queued/in_progress->pending + reseed coherente (requiere --yes)
	node scripts/precheck-refresh-timestamp-offset.mjs --json  # precheck de offset para migración timestamptz (guardrail)
...
- Panel `/admin/catalog-refresh`:
  ...
  - Alertas en campanita desplegable (dropdown) enfocadas en refresh/catálogo (`stale_brand`, `catalog_stuck`, `catalog_queue_drift`, `processing_no_progress`); acciones inline de `Forzar refresh`, `Resume fuerte` y `Reconciliar cola`.
  - Alertas críticas (`criticalOperationalAlerts`) no truncadas y priorizadas antes del límite visual; `processing` overdue sin progreso ya no se oculta por `alertLimit`.
  - Forzado por marca migra a endpoint asíncrono `POST /api/admin/catalog-refresh/force` para evitar timeout de 60s.
  - Bloque “Faltan X para 100%” muestra mini barra por marca con progreso real del run (`completed/total`, fallos y status).
  - El badge `overdue` en tabla usa criterio operativo semanal (`operationalOverdue`) y no scheduler legacy.
  - Bloque de cierre a 100%: lista de marcas faltantes (`operationalMissingBrands`) con acción `Forzar`, marca más antigua (`oldestOperationalRefresh`) y estado de jobs (`/api/admin/queue-health`).
  - Estado de jobs muestra guardrails (`heartbeatMissing`, `activeHung`, `queueDriftDetected`, `aggressiveRecoveryRequired`), breakdown de zombies (`completed`, `failed_terminal`, `run_not_processing`) y botón de reconciliación manual.
  - Archivo automático seguro: endpoint `POST /api/admin/catalog-refresh/archive-candidates` (dry-run/apply), `archiveAlerts` y `archiveCandidates` en `GET /api/admin/catalog-refresh/state`, KPI de archivo en `summary` y acciones `Dry-run/Aplicar` desde la UI.
  - Reglas de archivo automáticas: `404_real` con evidencia fuerte (ratio + recheck HTTP) y `no_products_validated` con doble validación separada por >=24h.
  - Al archivar marca: `isActive=false`, runs catálogo/enrichment se cierran, items runnable se normalizan a terminal, jobs se limpian y productos quedan `status='archived'`.
  - Tabla de marcas usa estado único reconciliado (`catalogStatus`), con tooltip técnico `run=<...>, refresh=<...>, source=<...>`.
  - Tarjeta “Estado 100% real”: solo marca `Sí` cuando `operationalCoverageExact=true` y `operationalHealthOk=true`; si no, expone `realGapReasons`.
...
- OAuth callback hardening: si Descope vuelve a `/sign-in` con `?code=` o `?err=`, la UI espera bootstrap de sesión y, si no hay token local, ejecuta `sdk.oauth.exchange(code)` una sola vez por código. `E061301` se trata como carrera recuperable (sin botón `Reintentar` y sin error visible), con poll corto de sesión y limpieza automática de URL si no aparece token. Además, `next/returnTo` se sanitiza para bloquear destinos de auth (`/sign-in`, `/auth/callback`) y query params OAuth (`code`, `err`, `error`, `descope-login-flow`) para evitar loops.
...
- Autorización: header `Authorization: Bearer <ADMIN_TOKEN>` (fallback a `NEXTAUTH_SECRET` si no se define). La protección es server-side en el handler (`validateAdminRequest`), sin depender de `middleware.ts`.
...
- `POST /api/admin/catalog-extractor/run`: ejecuta extracción/reanuda una marca (body: `{ brandId, limit, resume, strongResume, drainOnRun }`), y responde además `recoveredQueued`, `recoveredInProgress`, `reenqueued`.
- `POST /api/admin/catalog-extractor/reconcile`: reconciliación DB↔Redis de cola catálogo (global o por `brandId`/`runId`; soporta `dryRun`).
...
- `/api/admin/catalog-refresh/cron?mode=light` cada 5 minutos (`*/5 * * * *`).
- `/api/admin/catalog-refresh/cron?mode=heavy` cada 1 hora (`0 * * * *`).
