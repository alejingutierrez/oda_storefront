...
Semántica KPI (admin refresh): `freshBrands`/`staleBrands` ahora representan cobertura operativa automática (último intento finalizado en ventana semanal para marcas auto‑elegibles). Además, `GET /api/admin/catalog-refresh/state` expone `operationalFreshBrands`, `qualityFreshBrands`, `operationalStaleBrands`, `qualityStaleBrands`, `autoEligibleBrands`, `staleBreakdown`, `operationalMissingBrands[]` y `oldestOperationalRefresh`; por marca agrega `schedulerDue`, `operationalOverdue`, `lastOperationalAt`, `lastForceAttemptAt` y `lastForceResult`. Cada marca faltante incluye `runProgress` (runId/status/completed/failed/pending/progressPct) para mini barras de avance en UI. Por defecto el endpoint devuelve alertas solo de refresh/catálogo, incluyendo `stale_auto_recovering` y `stale_no_refs`; para diagnóstico extendido se puede usar `?includeEnrichmentAlerts=true`.
...
- Panel `/admin/catalog-refresh`:
  ...
  - Alertas en campanita desplegable (dropdown) enfocadas en refresh/catálogo (`stale_brand`, `catalog_stuck`); acciones inline de `Forzar refresh` y `Reanudar` catálogo.
