...
Semántica KPI (admin refresh): `freshBrands`/`staleBrands` ahora representan cobertura operativa automática (último intento finalizado en ventana semanal para marcas auto‑elegibles). Además, `GET /api/admin/catalog-refresh/state` expone `operationalFreshBrands`, `qualityFreshBrands`, `operationalStaleBrands`, `qualityStaleBrands`, `autoEligibleBrands`, `staleBreakdown`, `operationalMissingBrands[]`, `oldestOperationalRefresh`, y `diagnostics` con `queueDrift`, `heartbeat` y `progress` efectivo reciente. Por marca agrega `schedulerDue`, `operationalOverdue`, `lastOperationalAt`, `lastForceAttemptAt` y `lastForceResult`. Cada marca faltante incluye `runProgress` (runId/status/completed/failed/pending/progressPct). Las alertas priorizan `processing` con progreso real reciente vs `processing` sin progreso y drift.
...
- Health: `GET /api/admin/queue-health` (jobCounts BullMQ + workerAlive + `flags.heartbeatMissing/activeHung/queueDriftDetected` + `drift` catálogo).
...
- Panel `/admin/catalog-refresh`:
  ...
  - Estado de jobs muestra guardrails (`heartbeatMissing`, `activeHung`, `queueDriftDetected`) y botón de reconciliación manual.
