...
Remediación masiva balanceada: `POST /api/admin/catalog-refresh/remediate` (request `{ dryRun, strategy, limit, minNoProgressMinutes, pauseOverCapTarget }`) ejecuta reconcile previo + resume por lotes + pausa selectiva sobre cap. El cron adaptable (`/api/admin/catalog-refresh/cron?mode=light|heavy`) incluye bloque `stuckRemediation` y `GET /api/admin/catalog-refresh/state` agrega `summary.processingNoProgressCount`, `summary.processingNoProgressTop`, `summary.processingNoProgressOverflow` y `diagnostics.processingNoProgress`.
...
node scripts/backfill-variant-images-to-blob.mjs  # migra `variants.images[]` externos a Blob por lotes
	...
	node scripts/rebaseline-catalog-refresh-metadata.mjs --apply  # aplica el rebaseline en DB (usar después de validar dry-run)
		...
		node scripts/monitor-catalog-refresh-ops.mjs --base-url=https://oda-moda.vercel.app --interval-minutes=5 --iterations=0 --run-cron --cron-mode=light --remediate-cooldown-minutes=30 --remediate-limit=60 --remediate-min-no-progress-minutes=20 --remediate-pause-over-cap-target=64 --guardrail-consecutive-cycles=2 --reconcile-job-scan-limit=2500 --reconcile-reenqueue-limit=800 --reconcile-include-active-analysis=true --state-file=../../reports/catalog_refresh_diagnostics/monitor-state.json  # monitoreo continuo 24/7 + auto-reconcile/remediate con guardrails y snapshots
