...
## Done (2026-02)
- MC-125 | done | P1 | Catálogo público: refinamientos PLP `/catalogo` (mobile + precio + cards) | Desktop: categorías top 10 con scroll + hint. Mobile: dock sticky (no tapa contenido) + toggle de layout (1/2 columnas, 1:1/4:5/3:4). Precio: histograma más legible + slider touch + rangos múltiples disjuntos (`price_range=min:max` repetible). Cards: icono cesta en glass (hover desktop, visible mobile) + crossfade sin flicker. Comparador mobile 1:1.
