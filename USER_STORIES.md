...
- Métricas/SLO:
  ...
  - `cold`: p95 `< 3s`
- Estado: **done (2026-02-20)**.
- Seguimiento (2026-02-25):
  - Mobile draft de filtros: `CatalogoFiltersPanel` calcula `priceBoundsFetchKey` con estado efectivo (`currentParamsString`) y dispara `mode=full` automáticamente al abrir el bloque de filtros, evitando histograma vacío hasta el primer gesto del slider.
...
- Seguimiento (2026-02-25, PLPs fijas precalculadas semanalmente):
  - `catalog-data` extrae el cómputo de insights completos a función interna reutilizable y agrega `getCatalogPriceInsightsFixedPlp(...)` con namespace de caché separado y TTL semanal (`CATALOG_FIXED_PLP_PRICE_INSIGHTS_REVALIDATE_SECONDS`, default `604800`).
  - `CatalogoView` detecta `isFixedPlpBaseRequest` cuando hay contexto `plp` y no existen filtros no-lockeados distintos de `page/sort`; en ese caso siembra SSR con `bounds + histogram + stats` (sin loading inicial).
  - Fallback resiliente: si falla el SSR fixed insights, la vista cae de inmediato a `getCatalogPriceBounds` y el cliente conserva flujo lazy dinámico de `mode=full`.
  - Operación: nuevo cron `GET /api/admin/catalog-price-insights/cron` con auth de cron/admin, fuente `plp_seo_pages.path`, sharding determinista por slot (`CATALOG_FIXED_PLP_PRECOMPUTE_SLOT_COUNT`, default `168`) y tope por corrida (`CATALOG_FIXED_PLP_PRECOMPUTE_MAX_PATHS_PER_RUN`, default `12`).
  - Programación: `apps/web/vercel.json` agrega ejecución horaria; cada PLP fija se refresca aproximadamente una vez por semana.
...
- Alcance: Cron `/api/admin/catalog-refresh/cron` selecciona marcas vencidas con jitter y límites por batch; inicia `catalog_runs/items` para refresco completo; soporta concurrencia por marca configurable (`CATALOG_REFRESH_BRAND_CONCURRENCY`) y overrides por request (`maxBrands`, `brandConcurrency`, `maxRuntimeMs`) para tuning operativo; guarda métricas y estado en `brands.metadata.catalog_refresh`; re‑encola URLs fallidas recientes; al completar, **crea** un `product_enrichment_run` en `paused` (mode `new_products`) para productos nuevos sin `metadata.enrichment` (queda listo para reanudar manualmente, sin drenar automáticamente). Panel `/admin/catalog-refresh` muestra KPI dual (cobertura operativa automática vs calidad), cobertura discovery, cambios de precio/stock y alertas con acciones. Hardening adicional 2026-02-23: endpoint asíncrono `POST /api/admin/catalog-refresh/force` para forzar por marca sin timeout 504, fallback por fuentes (`latest run refs` -> `products.sourceUrl` -> `discovery` acotado), `state` agrega `runProgress`/`lastForceResult`/`lastForceAttemptAt` en faltantes, alertas `stale_auto_recovering` y `stale_no_refs`, auto-remediación programada de stale (`CATALOG_REFRESH_AUTO_REMEDIATE_*`) y auto-conversión `manualReview` por evidencia (`CATALOG_MANUAL_REVIEW_AUTOCLEAR_*`). Hardening 2026-03-01: cap global de presión (`CATALOG_REFRESH_MAX_ACTIVE_RUNS`) y auto-reconcile de drift DB↔BullMQ pre-batch (`CATALOG_REFRESH_AUTO_RECONCILE*`) con telemetría operativa adicional en respuesta de cron. Hardening 2026-03-02: remediación balanceada para runs estancados (`POST /api/admin/catalog-refresh/remediate`), agregación top N de alertas `processing` sin progreso (`CATALOG_ALERT_TOP_BRANDS`) y gracia anti-falso-zombie en active (`CATALOG_QUEUE_ACTIVE_COMPLETED_GRACE_SECONDS`). Rollout prod directo 2026-03-02: deploy `dpl_HjMgR8euZovhhNLAqXJreDBMcikX` (alias `https://oda-moda.vercel.app`), dry-run/apply balanceado ejecutado (`resumed=30`, `paused=30`, `requeued=3000`), monitor operativo `apps/web/scripts/monitor-catalog-refresh-ops.mjs` en ejecución real con snapshot en `reports/catalog_refresh_diagnostics/2026-03-02T03-32-14-983Z-catalog-refresh-ops.json`, y reconcile reforzado (`jobScanLimit=2500`, `reenqueueLimit=800`, `includeActiveAnalysis=true`) para estabilizar drift crítico. Hardening 2026-03-02 (anti-rebote): `runCatalogRefreshStuckRemediation` agrega post-reconcile automático (`postReconciled/postReconcileResult`) para limpiar drift generado por la propia remediación, y el monitor continuo incorpora cooldown (`--remediate-cooldown-minutes`, env `CATALOG_REFRESH_MONITOR_REMEDIATE_COOLDOWN_MINUTES`, default 30) para evitar acciones repetitivas de requeue en cada ciclo; además, el monitor expone configuración reforzada de reconcile (`--reconcile-job-scan-limit`, `--reconcile-reenqueue-limit`, `--reconcile-include-active-analysis`) y timeout por request (`--request-timeout-ms`, env `CATALOG_REFRESH_MONITOR_REQUEST_TIMEOUT_MS`, default `45000`) para evitar cuelgues en ciclos largos. Hardening 2026-03-02 (autónomo 99%): `runCatalogRefreshStuckRemediation` incorpora estrategia `aggressive_tail_close` (cierre seguro de runs en 99% sin progreso), `runCatalogRefreshBatch` calcula capacidad por `processing` real, `state` agrega telemetría de alto progreso sin avance y capacidad de scheduling (`processing/paused/blocked/schedulingCapacityRemaining`), `catalog-extractor/drain` añade `dryRun`/micro-drain cron con lock anti-overlap y métricas de cierre forzado, y el monitor 24/7 queda con lock de instancia única + guardrail por heartbeat faltante (`drain + reconcile + cron light`). Validación prod en ventana 15 min: tras primer ciclo correctivo, se sostuvo `waitingItemNotQueued=0`, `runsRunnableWithoutQueueLoad=0` y `activeZombieCriticalCount=0`; con reconcile posterior aplicado se limpió rebote operativo (`waitingItemNotQueued 127 -> 0`) manteniendo workers online. VTEX usa API `/api/catalog_system/pub/products/search` y evita depender de sitemaps truncados. Se mantiene gate de éxito (`CATALOG_REFRESH_MAX_FAILED_ITEMS=30`, `CATALOG_REFRESH_MAX_FAILED_RATE=0.10`) y backoff exponencial (`CATALOG_REFRESH_FAILED_BACKOFF_BASE_HOURS=6`, `CATALOG_REFRESH_FAILED_BACKOFF_MAX_HOURS=72`).
- CA: Cada marca activa con `siteUrl` se refresca al menos cada 7 días; el cron no sobrecarga el sistema (límite de marcas + tiempo máx por ejecución + concurrencia configurable + cap global de runs activos); los nuevos productos quedan **en cola** para enriquecimiento (run `paused`, sin ejecución automática para evitar cuota de OpenAI); cambios de precio/stock alimentan `price_history`/`stock_history`; el panel separa cobertura operativa automática (usa `lastFinishedAt`/`lastCompletedAt`) y calidad (usa `lastCompletedAt`), con `manualReview` fuera del denominador automático y breakdown explícito (`processing`, `failed`, `completed_stale`, `unknown`, `manual_review`) manteniendo alias `no_status` por compatibilidad; alertas por default no incluyen enrichment; forzar una marca retorna `202` y no timeout, y el bloque de faltantes muestra mini barra de progreso real por run; `manualReview=true` se auto-limpia al cumplir evidencia operativa cerrada (2 runs exitosos en 21 días, sin reason bloqueante); no se crean tablas nuevas (solo metadata y tablas existentes).
...
- Estado: **done (2026-02-04) · hardening (2026-02-08, 2026-02-11, 2026-02-23: perfil agresivo + fix parse query params opcionales en `/api/admin/catalog-refresh/cron` + gate 30/10 + backoff exponencial + KPI dual + script de rebaseline metadata + alertas por contexto refresh con campanita y overdue operativo + force async sin 504 + mini progreso por faltantes + auto-remediación stale + auto-clear manualReview por evidencia; 2026-03-01: cap global activo + auto-reconcile pre-batch + semántica stale `completed_stale/unknown` + payload operativo de cron ampliado; 2026-03-02: timeout HTTP extractor/PDP ajustado (`CATALOG_HTTP_TIMEOUT_MS`, `CATALOG_PDP_HTML_TIMEOUT_MS`), deploy prod y monitoreo post-release con drift operativo reducido; 2026-03-02 ops recovery: aislamiento de run con abort storm en `yerbabuena`, relanzamiento de worker con `CATALOG_WORKER_CONCURRENCY=8` y descenso medido de backlog en ventana corta; 2026-03-02 estabilización balanceada: endpoint `remediate`, agregación top N/overflow en alertas, severidad de drift por `activeZombieCriticalCount` y script `monitor-catalog-refresh-ops.mjs`; 2026-03-02 rollout prod directo: deploy `dpl_HjMgR8euZovhhNLAqXJreDBMcikX`, dry-run/apply balanceado y reconcile reforzado con `waitingItemNotQueued` estabilizado en `0` y `processingRunsWithoutRecentProgress` bajando de `155` a `64` en la ventana inicial; 2026-03-02 anti-rebote: deploy `dpl_4hEtNEHfJyNvUATJB2FyzVtJLzDH` con post-reconcile automático y cooldown del monitor, validado con `postReconciled=true` y drift post-remediación en `0`; 2026-03-02 monitoreo reforzado: deploy `dpl_F9JBYh2iomy5fkAw73zqCEhyxxv5` y ventana de 15 min con cooldown/reconcile reforzado manteniendo drift crítico en `0`; 2026-03-02 timeout monitor: deploy `dpl_9hXDSfrqKqxpZmT9e2QmBkgy5f6y` con timeout por request y reconcile reforzado, manteniendo drift crítico en `0` tras acción correctiva; 2026-03-02 autónomo 99%: cierre agresivo de tails + gating por `processing` + monitor 24/7 con lock/heartbeat guardrail + micro-drain cron)**.
- Actualización operativa 2026-03-02 (hotfix desbloqueo 24/7): cron adaptable en `GET /api/admin/catalog-refresh/cron?mode=light|heavy` (default `light`), `maxDuration=300` en route cron, tareas pesadas aisladas a `mode=heavy` dentro de `runCatalogRefreshBatch`, publicación explícita de `POST /api/admin/catalog-refresh/remediate`, y monitoreo continuo reforzado en `apps/web/scripts/monitor-catalog-refresh-ops.mjs` con remediación agresiva (`limit=60`, `minNoProgressMinutes=20`, `pauseOverCapTarget=64`), guardrail por ciclos consecutivos de `activeHung/zombieCritical`, reconcile aplicado por drift/runnable sin carga y persistencia de estado en `reports/catalog_refresh_diagnostics/monitor-state.json`.
- Actualización operativa 2026-03-02 (diagnóstico amplio con baja intervención): `monitor-catalog-refresh-ops.mjs` ahora consume `diagnostics.processingNoProgress.all` para cubrir todas las marcas estancadas, calcula persistencia por run entre ciclos (`consecutiveNoProgressCycles`) y publica `noProgressInsights` (buckets de progreso/antigüedad + top persistentes). El monitor también tolera caídas parciales de `/api/admin/catalog-refresh/state` y `/api/admin/queue-health` sin cortar el ciclo, registrando snapshots con `rootCause=state_unavailable|queue_health_unavailable`. En modo `minimal`, limita remediación a near-close persistentes (>= `CATALOG_REFRESH_MONITOR_NEAR_CLOSE_MIN_CONSECUTIVE_CYCLES`) para evitar intervención excesiva.
- Actualización operativa 2026-03-03 (plan integral high-progress): `runCatalogRefreshStuckRemediation` añade estrategia `safe_fast_high_progress` (criterio `>=95%`, `pending<=10`, `completedRecent=0` en 5m, sin criterio de edad), con reconcile + micro-drain por `runId`, terminalización opcional del remanente (`safe_fast_high_progress_close`) y cierre final (`completed|failed`). `POST /api/admin/catalog-refresh/remediate` amplía request/response para high-progress; `catalog-extractor/drain` ejecuta evaluación post-drain con esa estrategia; `GET /api/admin/catalog-refresh/state` agrega `summary.throughput`, `summary.sla` y `summary.highProgressClosableTop`; `GET /api/admin/queue-health` agrega throughput y `catalogNoProgressMinutes/queueBurnEstimateMinutes`. `runCatalogRefreshBatch` incorpora reseed continuo controlado al llegar a 100% de cobertura con gates de salud de cola/heartbeat. `monitor-catalog-refresh-ops.mjs` incorpora cierre autónomo high-progress por ciclo, cooldown corto y alertas SLA (`<120 items/5m` warning, `<60 items/5m` crítica).
- Actualización operativa 2026-03-02 (autocierre >95% + 60 min): la política por defecto de `aggressive_tail_close` queda en `tailProgressPct=95`, `tailStaleMinutes=60` y `tailMaxRemaining=0` para cerrar automáticamente runs near-complete que no avanzan por más de una hora. Se elimina el bypass de stale para `runId` (también respeta la ventana), y `state` alinea `highProgressNoProgress` con umbral `95` para alertar exactamente el caso de negocio.
...
- Alcance: Endurecer drenado y reconciliación: `/api/admin/product-enrichment/drain` detecta runs `created_by=catalog_refresh` con `auto_start=false` y los pasa a `paused` (con `blockReason=auto_start_disabled`), y `runCatalogRefreshBatch` aplica la misma normalización para corridas heredadas en `processing`. Además, al reanudar manualmente por `POST /api/admin/product-enrichment/run` (`resume=true`), la metadata del run se actualiza a `auto_start=true`. El comportamiento se puede invertir por env con `CATALOG_REFRESH_ENRICH_AUTO_START=true` para auto-procesar sin intervención (crear nuevas corridas en `processing` y reanudar corridas legacy `auto_start=false`).
- CA: Con `CATALOG_REFRESH_ENRICH_AUTO_START=false`, un run `catalog_refresh` con `auto_start=false` no permanece en `processing` por cron; queda/permanece en `paused` hasta reanudación explícita. Con `CATALOG_REFRESH_ENRICH_AUTO_START=true`, el refresh semanal crea corridas en `processing` y el cron de drain las procesa automáticamente.
...
### MC-110 Seguimiento continuo de estabilización (remediación balanceada 2)
- Historia: Como operador de catálogo, quiero que los runs pausados por sobrecap vuelvan a entrar al flujo en tandas controladas, para reducir estancamiento sin romper la cola.
- Alcance:
  - `runCatalogRefreshStuckRemediation` ahora evalúa `processing|paused|blocked` para reanudar runs con trabajo pendiente (excluyendo bloqueos manual review), priorizando `paused`.
  - Se añadió guardrail de presión: `resumeLimit` se acota por headroom de `processing` (`pauseOverCapTarget - processingRunCount`), de modo que si `processing` está sobre target la remediación no reanuda más runs y solo pausa para volver al rango objetivo.
  - `pauseOverCap` se calcula contra `processing` real, no contra el total activo (`processing+paused+blocked`), para evitar espiral de pausas.
  - `runCatalogRefreshBatch` puede ejecutar remediación aunque el umbral de `processingNoProgress` no dispare si existe backlog `paused` con `blockReason=stuck_over_cap`.
  - `monitor-catalog-refresh-ops.mjs` persiste estado (`lastRemediateAt/lastCronAt`) en archivo para respetar cooldown entre ejecuciones separadas.
- Criterios de aceptación:
  - Con backlog `paused(stuck_over_cap)`, la remediación balanceada reanuda parte de esos runs y disminuye el conteo de pausados.
  - En ejecuciones secuenciales del monitor dentro del cooldown, la acción aparece como `remediate_skipped_cooldown`.
  - Drift crítico se mantiene en 0 tras reconcile/remediate (`waitingItemNotQueued`, `runsRunnableWithoutQueueLoad`, `activeZombieCriticalCount`).
- Evidencia de validación en prod (2026-03-02):
  - Estado por status en DB pasó de `paused=121 / processing=38` a `paused=82 / processing=77` tras ciclos de remediación.
  - Con el guardrail de `resumeLimit` desplegado (`dpl_Eqrao4Kjb7hhqF9e9iD7Czvd9pxK`), un ciclo de cron ejecutó `resumed=0, paused=30`, bajando `processing 96 -> 69` y subiendo `paused 63 -> 90`.
